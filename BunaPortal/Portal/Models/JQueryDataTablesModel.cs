...
namespace CyroTechPortal
{
    ...
    public class JQueryDataTablesModel
    {
        ...
        public ReadOnlyCollection<FilterField> filterparams_ { get; set; }
        /// <summary>
        ...
        /// </summary>
        public Collection<string> sSearch_ { get; set; }
        public ReadOnlyCollection<string> sColumns_ { get; set; }
        ...
        public ReadOnlyCollection<FilterField> GetSortedColumns()
        {
            if (!iSortingCols.HasValue)
            {
                // Return an empty collection since it's easier to work with when verifying against
                return new ReadOnlyCollection<FilterField>(new List<FilterField>());
            }

            var sortedColumns = new List<FilterField>();
            for (int i = 0; i < iSortingCols.Value; i++)
            {
                sortedColumns.Add(new FilterField { Property = mDataProp_[iSortCol_[i]], Value = sSortDir_[i].Replace("asc","Ascending").Replace("desc","Descending") });
            }

            return sortedColumns.AsReadOnly();
        }

        public ReadOnlyCollection<FilterField> GetColumnsFilters()
        {
            var filteredColumns = new List<FilterField>();
            var par = filterparams_;
            var col = sColumns_;
            filteredColumns = Common.Common.GetFilters(new ReadOnlyCollection<string>(sSearch_), mDataProp_);
            return filteredColumns.AsReadOnly();
        }
    }
