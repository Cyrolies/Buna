...
var lazyload = {
  ...
  renderLazyLoad: function renderLazyLoad(data, startDate, endDate) {
    ...
    //used to get the current day highlighted
  ...
  renderLazyLoadGrid: function renderLazyLoadGrid(data, startDate, endDate) {
    ...
    for (var i = 0; i <= num; i += 1) {
      ...
      if (this.isEqual(sDate, lastDay)) {
        templateGridCol.css('border-color', 'yellow');
      }
  ...
  renderLazyLoadContainer: function renderLazyLoadContainer() {
    ...
    templateWrapper.append(template).append(templateLoading);
  ...
  renderLazyLoadEvents: function renderLazyLoadEvents(data) {
    ...
    data.forEach(function (element) {
      ...
      var itemEndDate = new Date(element.enddate);
    ...
    //  var tourWidth = (parseInt(element.minNight, 10) + 1) * this.config.cellWidth;
      var tourWidth = (this.dateDiffInDays(itemStartDate,itemEndDate)) * this.config.cellWidth;
     ...
     // alert(JSON.stringify(element));
      ...
      var title = element.title;// + ' Gece';

      var eventBlock = $$1('<div>', {
        class: this.format('gantt-event-block {0}', barType),
        width: tourWidth + 'px',
        target: '_blank'
      }).text(title).css('line-height', this.config.cellHeight - 19 + 'px').data('tooltip', this.tooltipView(tooltipData));
      ...
      var eventIcon = $$1('<div class="gantt-event-icon"><div class="' + barType + '"></div></div>');
  ...
  createClass(Gantt, [{
    ...
    value: function renderGrid(num) {
      ...
      for (var i = 0; i <= num; i += 1) {
        ...
        if (startDate.getDate() === currentDay && month === currentMonth) {
          
          templateGridCol.css('background-color', '#8A8A93');
        }
      ...
      data.forEach(function (el) {
        var templateGridRow = $$1('<div>', { class: 'gantt-grid-row', width: totalWidth, height: this.config.cellHeight });
        if (el.url == '136') //Grahamstown
        {
          templateGridRow = $$1('<div>', { class: 'gantt-grid-row-grahamstown', width: totalWidth, height: this.config.cellHeight });
        }
        if (el.url == '137') //Port Alfred
        {
          templateGridRow = $$1('<div>', { class: 'gantt-grid-row-portalfred', width: totalWidth, height: this.config.cellHeight });
        }
        if (el.url == '138') //Fort Hare
        {
          templateGridRow = $$1('<div>', { class: 'gantt-grid-row-forthare', width: totalWidth, height: this.config.cellHeight });
        }
        templateGridRows.append(templateGridRow);
      }, this);
    ...
    value: function renderEvents(num) {
      ...
      data.forEach(function (el) {
        ...
        var itemEndDate = new Date(el.enddate);
        var templateEventRow = $$1('<div>', { class: 'gantt-event-row', width: this.totalWidth });
        var templateEvent = $$1('<div>', { class: 'gantt-event' });
        //if enddate is not specified then keep setting it to 14 days ahead
        //alert(itemEndDate.toString());
        //if (itemEndDate == null || itemEndDate.length == 0)
        //{
        //  var dtCurrent = new Date();
        //  itemEndDate.setDate(dtCurrent.getDate() + 14)
        //}
        //else
        //{
          itemEndDate.setDate(itemEndDate.getDate() + 1)
        //}
      //  alert(itemEndDate.getDate().localeFormat());
      //  alert(itemEndDate.setDate(itemEndDate.getDate() + 1));
       // var tourWidth = (parseInt(el.minNight, 10) + 1) * this.config.cellWidth;
        var tourWidth = (this.dateDiffInDays(itemStartDate, itemEndDate)) * this.config.cellWidth;
        var remDay = this.dateDiffInDays(this.config.startDate, itemStartDate);
       
        var tooltipData = $$1.extend(el);
     ...
     ///   alert(barType);
        var title = el.title;// + ' Gece';
       // alert(title);
        var eventBlock = $$1('<div>', {
          class: this.format('gantt-event-block {0}', barType),
          width: tourWidth + 'px',
          target: '_blank',
          ondblclick:'edit('+el.id+')'
        }).text(title).css('line-height', this.config.cellHeight - 19 + 'px').data('tooltip', this.tooltipView(tooltipData));


        //var eventBlock = $$1('<a>', {
        //  class: 'gantt-event-block ' + barType,
        //  width: tourWidth + 'px',
        //  href: '' + el.url,
        //  target: '_blank'
        //}).text(title).css('line-height', this.config.cellHeight - 19 + 'px').data('tooltip', this.tooltipView(tooltipData));
    ...
    key: 'dateDiffInDays',
      ...
      dayTotal = Math.floor(((utc2) - (utc1)) / MS_PER_DAY);
        //if (dayTotal == 0) {
        //  return 1;
        //}
        //else {
    ...
    value: function tooltipView(data) {
      ...
      if (data.image != null) {
        var template = '' + '<div class="gantt-tooltip">' + '    <div class="tooltip-content">' + '        <img src="{0}" alt="tooltip-img">' + '        <span class="title">{1}</span>' + '        <div class="desc">{2}</div>' + '        </div>' + '    </div>' + '</div>';
        var html = this.format(template, data.image, data.title, data.desc);
      }
      else {
        
        var template = '' + '<div class="gantt-tooltip">' + '<div class="tooltip-content">' + '<span class="title">{0}</span>' + '<div class="desc">{1}</div> ' + '</div>' + '</div>';
        var html = this.format(template,data.title, data.desc);
