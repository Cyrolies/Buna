...
<div class="dj-megamenu-wrapper" <?php echo $isJoomla4 ? ' data-joomla4' : '' ?>>
	<?php if ($params->get('fixed')) { ?>
		<div id="dj-megamenu<?php echo $module->id; ?>sticky" class="dj-megamenu <?php echo 'dj-megamenu-' . $params->get('theme') . ' ' . $custom_colors . ' ' . $class_sfx; ?> dj-megamenu-sticky" style="display: none;">
	...
	<?php } ?>
	<ul id="dj-megamenu<?php echo $module->id; ?>" class="dj-megamenu <?php echo ' dj-fa-' . $params->get('fa') . ' ' . 'dj-megamenu-' . $params->get('theme') . ' ' . $custom_colors . ' ' . $params->get('orientation') . 'Menu ' . $class_sfx; ?>" <?php echo !empty($options) ? "data-options='" . $options . "'" : ""; ?> data-trigger="<?php echo (int)$params->get('width') ?>">
		<?php
		$first = true;
		foreach ($list as $index => &$item) :
			...
			if ($first) {
				...
				echo '<div class="dj-subcol" style="width:' . $item_params->get('djmegamenu-column_width') . '"><ul class="dj-submenu">';
				$class .= ' first';
			}
			...
			if ($item->deeper) {
				...
				if ($item->level > $startLevel && $expand[$item->id] == 'tree') {
					...
					echo '<div class="dj-subcol" style="width:' . $item_params->get('djmegamenu-first_column_width') . '"><ul class="dj-submenu">';
				}
				$first = true;
			}
		...
		?></ul>
	<?php
	if ($mobilemenu == '1') { //SELECT 
		$data_label = $module->title;
	?>
		<div id="dj-megamenu<?php echo $module->id; ?>mobile" class="dj-megamenu-select <?php echo 'dj-megamenu-select-' . $params->get('mobiletheme') . ' select-' . $params->get('select_type') . ' ' . $class_sfx; ?>" data-label="<?php echo $data_label; ?>">
			<?php require(JModuleHelper::getLayoutPath('mod_djmegamenu', 'default_btn')); ?>
			<?php /*<label class="hidden" aria-hidden="true" for="dj-megamenu<?php echo $module->id; ?>select"><?php echo $module->title ?></label>*/?>
		</div>
	<?php } else if ($mobilemenu == '2') { //OFFCANVAS ?>
		<div id="dj-megamenu<?php echo $module->id; ?>mobile" class="dj-megamenu-offcanvas <?php echo 'dj-megamenu-offcanvas-' . $params->get('mobiletheme') . ' ' . $class_sfx; ?>">
			<?php require(JModuleHelper::getLayoutPath('mod_djmegamenu', 'default_btn')); ?>
			<aside id="dj-megamenu<?php echo $module->id; ?>offcanvas" class="dj-offcanvas <?php echo 'dj-fa-' . $params->get('fa') . ' ' . 'dj-offcanvas-' . $params->get('mobiletheme') . ' ' . $class_sfx; ?>" data-effect="<?php echo $params->get('offcanvas_effect') ?>">
